...
DMGraph.factory('$dmGraph', function ($rootScope, $dmPatient) {
    return function() {
        ...
        var self = this;
        this.plotbp = function () {
            setTimeout(function () {
                $("#bp_graph").height(100);
                if (self.bpsys_arr.length > 0) {
                    $.plot($("#bp_graph"), [self.bpsys_arr, self.bpdia_arr], self.patientModel.bp_flot_opts);
                }
            });
        };

        this.plotldl = function () {
            $("#ldl_graph").height(100);
            if (self.ldl_arr.length > 0) {
                $.plot($("#ldl_graph"), [self.ldl_arr], self.patientModel.ldl_flot_opts);
            }
        };
        this.plota1c = function () {
            $("#a1c_graph").height(100);
            if (self.a1c_arr.length > 0) {
                $.plot($("#a1c_graph"), [self.a1c_arr], self.patientModel.a1c_flot_opts);
            }
        };
    }
});